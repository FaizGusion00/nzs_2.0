...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    // Redirect non-payers to their role dashboard
    const r = user?.role;
    if (r === 'zass') { router.push('/zass/dashboard'); return; }
    if (r === 'exec_processing') { router.push('/exec/processing/dashboard'); return; }
    if (r === 'exec_finance') { router.push('/exec/finance/dashboard'); return; }
    if (r === 'admin' || r === 'super_admin') { router.push('/admin/dashboard'); return; }
    if (r === 'agent_ipis' || r === 'agent_ipt' || r === 'agent_consultant') { router.push('/agent/dashboard'); return; }
    if (r === 'amil') { router.push('/amil/dashboard'); return; }
  }, [isAuthenticated, user, router]);

  if (!isAuthenticated || !user) return null;
  const isPayer = user.role === 'payer_individual' || user.role === 'payer_company';
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-6 sm:mb-8">
            ...
            <Card className="cursor-pointer hover:shadow-lg transition-shadow" onClick={() => router.push('/dashboard/reports')}>
              <CardHeader>
                ...
                <CardDescription>
                  Self-service: Muat turun laporan bayaran
                </CardDescription>
              </CardHeader>
              <CardContent>
                <Button variant="outline" className="w-full">
