...
function PaymentPageContent() {
  ...
  const processPayment = async (
    ...
    const successParams = new URLSearchParams({
      ref: isGuest ? `RES-HA-2026-${Date.now().toString().slice(-4)}` : `LZS-${Date.now()}`,
      method: methodId,
      amount: amount.toString(),
    });

    if (bankCode) successParams.set('bank', bankCode);
    if (isGuest) successParams.set('guest', '1');
    if (guestPhone) successParams.set('phone', guestPhone);
    if (guestEmail) successParams.set('email', guestEmail);
              ...
              <Card className="sticky top-24">
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    {isGuest && (
                      <Badge variant="secondary">Checkout: Hamba Allah</Badge>
                    )}
